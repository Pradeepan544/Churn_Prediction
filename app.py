...
# Load the dataset used during training for consistent preprocessing
df_1 = pd.read_csv("first_telc.csv")

# Load the model
model = pickle.load(open("Churn_Prediction_Model.sav", "rb"))
...
def predict():
    ...
    # Combine with original dataframe to maintain consistent preprocessing
    ...
    # Ensure the columns match those seen during training
    missing_cols = set(model.feature_names_in_) - set(df_2_dummies.columns)
    for col in missing_cols:
        df_2_dummies[col] = 0

    df_2_dummies = df_2_dummies[model.feature_names_in_]

    # Predict using the model
    single = model.predict(df_2_dummies.tail(1))
    probablity = model.predict_proba(df_2_dummies.tail(1))[:,1]
